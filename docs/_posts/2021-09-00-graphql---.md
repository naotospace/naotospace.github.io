...
```bash
git clone https://github.com/MoonHighway/learning-graphql.git
cd learning-graphql/chapter-05/photo-share-api
npm init -y
npm install apollo-server graphql nodemon --save --legacy-peer-deps
```

nodemon:

ファイル変更を監視し、変更が行われる際にサーバを再起動する

以下をpackage.jsonに追加する
